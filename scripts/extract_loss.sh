...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    if [ ! -f ${feats_dir}/feats.done ]; then 
        ...
        python -m asr_evaluate.statistics.conformer_pseudo_loss ${_opts}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    echo "Stage 2: Sample selection"
    if [ ! -f ${feats_dir}/sampling.done ]; then
        python -m utils.sample_selection_by_loss ${feats_dir}/${report_name}.csv ${k} ${feats_dir}/sample.csv
